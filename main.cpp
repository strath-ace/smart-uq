...
int main_test_composition(){
    ...
    ranges[0][0] = -.5; ranges[0][1] = .5;
    ranges[1][0] = -.5; ranges[1][1] = .5;
...
//    std::vector<Chebyshev_Polynomial<double> > f_poly;
//    for(int j=0; j<nvar; j++){
//        f_poly.push_back(Chebyshev_Polynomial<double>(nvar,degree));
//    }
//    f_poly=f(x0);
...
//        std::vector<double> range = res[0].get_range();
//        std::vector<Chebyshev_Polynomial<double> > tmp_res;
//        for(int j=0; j<nvar; j++){
//            tmp_res.push_back(Chebyshev_Polynomial<double>(nvar,degree));
//            tmp_res[j] = res[j];
//        }
//        for(int j=0; j<nvar; j++)
//            res[j] = tmp_res[j] + step*f_poly[j].composition(tmp_res);
...
//    int nvar = 1;
//    int ndegree = 2;
//    Chebyshev_Polynomial<double> p(nvar,ndegree);

//    double dt = 0.01;
//    p.set_coeffs(0,dt/2.0);
//    p.set_coeffs(1,1.0);
//    p.set_coeffs(2,dt/2.0);

//    std::cout<<setprecision(16)<<p*p;
    //main_2BP();
